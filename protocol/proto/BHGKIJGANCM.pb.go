// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.28.1
// 	protoc        v5.26.0
// source: BHGKIJGANCM.proto

package proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type BHGKIJGANCM int32

const (
	BHGKIJGANCM_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_NONE             BHGKIJGANCM = 0
	BHGKIJGANCM_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_EFFECT           BHGKIJGANCM = 1
	BHGKIJGANCM_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_BONUS      BHGKIJGANCM = 2
	BHGKIJGANCM_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_TAX        BHGKIJGANCM = 3
	BHGKIJGANCM_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_UPGRADE    BHGKIJGANCM = 4
	BHGKIJGANCM_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_GAME_SETTLE      BHGKIJGANCM = 5
	BHGKIJGANCM_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_BUY_GOODS        BHGKIJGANCM = 6
	BHGKIJGANCM_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_CLICK            BHGKIJGANCM = 7
	BHGKIJGANCM_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_SOCIAL_EVENT     BHGKIJGANCM = 8
	BHGKIJGANCM_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_LIKE             BHGKIJGANCM = 9
	BHGKIJGANCM_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_QUIZ_GAME_SETTLE BHGKIJGANCM = 10
)

// Enum value maps for BHGKIJGANCM.
var (
	BHGKIJGANCM_name = map[int32]string{
		0:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_NONE",
		1:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_EFFECT",
		2:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_BONUS",
		3:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_TAX",
		4:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_UPGRADE",
		5:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_GAME_SETTLE",
		6:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_BUY_GOODS",
		7:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_CLICK",
		8:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_SOCIAL_EVENT",
		9:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_LIKE",
		10: "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_QUIZ_GAME_SETTLE",
	}
	BHGKIJGANCM_value = map[string]int32{
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_NONE":             0,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_EFFECT":           1,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_BONUS":      2,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_TAX":        3,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_UPGRADE":    4,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_GAME_SETTLE":      5,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_BUY_GOODS":        6,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_CLICK":            7,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_SOCIAL_EVENT":     8,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_LIKE":             9,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_QUIZ_GAME_SETTLE": 10,
	}
)

func (x BHGKIJGANCM) Enum() *BHGKIJGANCM {
	p := new(BHGKIJGANCM)
	*p = x
	return p
}

func (x BHGKIJGANCM) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (BHGKIJGANCM) Descriptor() protoreflect.EnumDescriptor {
	return file_BHGKIJGANCM_proto_enumTypes[0].Descriptor()
}

func (BHGKIJGANCM) Type() protoreflect.EnumType {
	return &file_BHGKIJGANCM_proto_enumTypes[0]
}

func (x BHGKIJGANCM) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use BHGKIJGANCM.Descriptor instead.
func (BHGKIJGANCM) EnumDescriptor() ([]byte, []int) {
	return file_BHGKIJGANCM_proto_rawDescGZIP(), []int{0}
}

var File_BHGKIJGANCM_proto protoreflect.FileDescriptor

var file_BHGKIJGANCM_proto_rawDesc = []byte{
	,
}

var (
	file_BHGKIJGANCM_proto_rawDescOnce sync.Once
	file_BHGKIJGANCM_proto_rawDescData = file_BHGKIJGANCM_proto_rawDesc
)

func file_BHGKIJGANCM_proto_rawDescGZIP() []byte {
	file_BHGKIJGANCM_proto_rawDescOnce.Do(func() {
		file_BHGKIJGANCM_proto_rawDescData = protoimpl.X.CompressGZIP(file_BHGKIJGANCM_proto_rawDescData)
	})
	return file_BHGKIJGANCM_proto_rawDescData
}

var file_BHGKIJGANCM_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_BHGKIJGANCM_proto_goTypes = []interface{}{
	(BHGKIJGANCM)(0), // 0: BHGKIJGANCM
}
var file_BHGKIJGANCM_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_BHGKIJGANCM_proto_init() }
func file_BHGKIJGANCM_proto_init() {
	if File_BHGKIJGANCM_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_BHGKIJGANCM_proto_rawDesc,
			NumEnums:      1,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_BHGKIJGANCM_proto_goTypes,
		DependencyIndexes: file_BHGKIJGANCM_proto_depIdxs,
		EnumInfos:         file_BHGKIJGANCM_proto_enumTypes,
	}.Build()
	File_BHGKIJGANCM_proto = out.File
	file_BHGKIJGANCM_proto_rawDesc = nil
	file_BHGKIJGANCM_proto_goTypes = nil
	file_BHGKIJGANCM_proto_depIdxs = nil
}
