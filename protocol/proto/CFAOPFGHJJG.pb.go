// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.28.1
// 	protoc        v5.26.0
// source: CFAOPFGHJJG.proto

package proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type CFAOPFGHJJG int32

const (
	CFAOPFGHJJG_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_NONE             CFAOPFGHJJG = 0
	CFAOPFGHJJG_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_EFFECT           CFAOPFGHJJG = 1
	CFAOPFGHJJG_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_BONUS      CFAOPFGHJJG = 2
	CFAOPFGHJJG_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_TAX        CFAOPFGHJJG = 3
	CFAOPFGHJJG_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_UPGRADE    CFAOPFGHJJG = 4
	CFAOPFGHJJG_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_GAME_SETTLE      CFAOPFGHJJG = 5
	CFAOPFGHJJG_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_BUY_GOODS        CFAOPFGHJJG = 6
	CFAOPFGHJJG_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_CLICK            CFAOPFGHJJG = 7
	CFAOPFGHJJG_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_SOCIAL_EVENT     CFAOPFGHJJG = 8
	CFAOPFGHJJG_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_LIKE             CFAOPFGHJJG = 9
	CFAOPFGHJJG_MONOPOLY_ACTION_RESULT_SOURCE_TYPE_QUIZ_GAME_SETTLE CFAOPFGHJJG = 10
)

// Enum value maps for CFAOPFGHJJG.
var (
	CFAOPFGHJJG_name = map[int32]string{
		0:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_NONE",
		1:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_EFFECT",
		2:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_BONUS",
		3:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_TAX",
		4:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_UPGRADE",
		5:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_GAME_SETTLE",
		6:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_BUY_GOODS",
		7:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_CLICK",
		8:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_SOCIAL_EVENT",
		9:  "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_LIKE",
		10: "MONOPOLY_ACTION_RESULT_SOURCE_TYPE_QUIZ_GAME_SETTLE",
	}
	CFAOPFGHJJG_value = map[string]int32{
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_NONE":             0,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_EFFECT":           1,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_BONUS":      2,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_TAX":        3,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_ASSET_UPGRADE":    4,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_GAME_SETTLE":      5,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_BUY_GOODS":        6,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_CLICK":            7,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_SOCIAL_EVENT":     8,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_LIKE":             9,
		"MONOPOLY_ACTION_RESULT_SOURCE_TYPE_QUIZ_GAME_SETTLE": 10,
	}
)

func (x CFAOPFGHJJG) Enum() *CFAOPFGHJJG {
	p := new(CFAOPFGHJJG)
	*p = x
	return p
}

func (x CFAOPFGHJJG) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (CFAOPFGHJJG) Descriptor() protoreflect.EnumDescriptor {
	return file_CFAOPFGHJJG_proto_enumTypes[0].Descriptor()
}

func (CFAOPFGHJJG) Type() protoreflect.EnumType {
	return &file_CFAOPFGHJJG_proto_enumTypes[0]
}

func (x CFAOPFGHJJG) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use CFAOPFGHJJG.Descriptor instead.
func (CFAOPFGHJJG) EnumDescriptor() ([]byte, []int) {
	return file_CFAOPFGHJJG_proto_rawDescGZIP(), []int{0}
}

var File_CFAOPFGHJJG_proto protoreflect.FileDescriptor

var file_CFAOPFGHJJG_proto_rawDesc = []byte{
	,
}

var (
	file_CFAOPFGHJJG_proto_rawDescOnce sync.Once
	file_CFAOPFGHJJG_proto_rawDescData = file_CFAOPFGHJJG_proto_rawDesc
)

func file_CFAOPFGHJJG_proto_rawDescGZIP() []byte {
	file_CFAOPFGHJJG_proto_rawDescOnce.Do(func() {
		file_CFAOPFGHJJG_proto_rawDescData = protoimpl.X.CompressGZIP(file_CFAOPFGHJJG_proto_rawDescData)
	})
	return file_CFAOPFGHJJG_proto_rawDescData
}

var file_CFAOPFGHJJG_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_CFAOPFGHJJG_proto_goTypes = []interface{}{
	(CFAOPFGHJJG)(0), // 0: CFAOPFGHJJG
}
var file_CFAOPFGHJJG_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_CFAOPFGHJJG_proto_init() }
func file_CFAOPFGHJJG_proto_init() {
	if File_CFAOPFGHJJG_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_CFAOPFGHJJG_proto_rawDesc,
			NumEnums:      1,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_CFAOPFGHJJG_proto_goTypes,
		DependencyIndexes: file_CFAOPFGHJJG_proto_depIdxs,
		EnumInfos:         file_CFAOPFGHJJG_proto_enumTypes,
	}.Build()
	File_CFAOPFGHJJG_proto = out.File
	file_CFAOPFGHJJG_proto_rawDesc = nil
	file_CFAOPFGHJJG_proto_goTypes = nil
	file_CFAOPFGHJJG_proto_depIdxs = nil
}
