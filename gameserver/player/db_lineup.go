...
func (g *GamePlayer) GetTrialAvatar(trialAvatarId uint32) {
	db := g.GetTrialLine()
	for _, avatar := range db.AvatarIdList {
		if avatar.AvatarId == 0 {
			isTrial := true
			if g.GetAvatarById(avatar.AvatarId) != nil {
				isTrial = false
			}
			avatar.AvatarId = trialAvatarId
			avatar.IsTrial = isTrial
			break
		}
	}
